...
export default function ProductCard({ p }: any) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[99999] bg-black flex flex-col">
      {/* X cerrar — fixed al viewport, SIEMPRE visible, con safe-area para iPhones con notch */}
      <button
        onClick={() => setOpen(false)}
        className="fixed z-[100000] right-3 md:right-4 w-14 h-14 md:w-12 md:h-12 rounded-full bg-black/85 ring-2 ring-white/40 hover:bg-black active:scale-95 text-white text-2xl shadow-2xl flex items-center justify-center transition"
    ...
    <>
      <div className="group bg-[#0f0f0f] border border-white/5 rounded-xl md:rounded-2xl overflow-hidden hover:border-white/10 hover:shadow-2xl transition-all duration-300">
        <div className="relative cursor-pointer" onClick={() => setOpen(true)}>
          <Swiper modules={[Pagination]} pagination={{ clickable: true }}>
            {p.images?.map((img: any, i: number) => (
              <SwiperSlide key={i}>
                <img
                  src={img.asset.url}
                  className="w-full h-40 md:h-64 object-cover group-hover:scale-105 transition duration-500"
                />
              </SwiperSlide>
            ))}
          </Swiper>
          ...
          {p.inStock && (
            <div className="absolute top-2 left-2 z-20">
              <span className="inline-flex w-fit items-center gap-1 rounded-full bg-green-600 px-2 py-0.5 md:px-3 md:py-1 text-[10px] md:text-[11px] font-semibold text-white">
                ✅ Stock
              </span>
            </div>
          )}
        ...
        <div className="p-2.5 md:p-6 flex flex-col gap-1 md:gap-2">
          <h2 className="text-[12px] md:text-sm font-medium text-white line-clamp-2 leading-tight">
            {p.name}
          </h2>
          ...
          <button
            onClick={handleWhatsapp}
            className="mt-1.5 md:mt-4 flex items-center justify-center gap-1 md:gap-2 text-green-400 hover:text-green-300 text-[11px] md:text-xs font-medium transition"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 32 32"
      ...
      {/* Modal en portal — escapa el stacking context del catálogo y queda sobre TODO (navbar, banner, botón WhatsApp) */}
